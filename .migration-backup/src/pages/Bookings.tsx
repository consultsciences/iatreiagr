...
const BookingsPage = () => {
  ...
  const loadBookings = async () => {
    ...
    const { data: claimed } = await supabase
      ...
      .select("id");
    if (claimed && claimed.length > 0) setClaimedCount(claimed.length);

    const { data, error } = await supabase
      .from("bookings")
      .select("*")
      .order("appointment_date", { ascending: false })
      .order("appointment_slot", { ascending: false });
    if (error) {
  ...
  const handleCancel = async () => {
    ...
    setCancelling(false);
    if (error) {
      toast({ title: "Αδυναμία ακύρωσης", description: error.message, variant: "destructive" });
      return;
    }
    if (!data || data.length === 0) {
      toast({
        title: "Δεν είναι δυνατή η ακύρωση",
        description: "Οι ακυρώσεις γίνονται δεκτές έως 24 ώρες πριν το ραντεβού.",
        variant: "destructive",
      });
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <Link to={`/doctor/${b.doctor_id}`} className={`text-base font-semibold hover:text-primary ${isCancelled ? "line-through" : "text-foreground"}`}>
                {b.doctor_name}
              </Link>
              {isCancelled && (
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        {claimedCount > 0 && (
          <div className="mt-4 flex items-start gap-2 rounded-lg border border-success/30 bg-success/10 p-3 text-sm">
            <CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-success" />
            <span>
              Προσθέσαμε {claimedCount} προηγούμενη κράτηση που είχατε κάνει με αυτό το email.
            </span>
          </div>
        )}
        ...
        {loading ? (
          ...
          <Card className="mt-8 p-12 text-center">
            ...
            <Button asChild className="mt-4">
              <Link to="/search"><SearchIcon className="mr-1.5 h-4 w-4" /> Αναζήτηση γιατρού</Link>
            </Button>
          </Card>
        ) : (
