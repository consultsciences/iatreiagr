...
const AdminDashboard = () => {
  ...
  const loadDoctors = useCallback(
    async (verified: boolean, page: number) => {
      ...
      if (search) {
        ...
        q = q.or(
          `full_name.ilike.${term},email.ilike.${term},specialty.ilike.${term},city.ilike.${term}`,
        );
      }
      ...
      if (error) {
        toast({
          ...
          description: `Δεν ήταν δυνατή η φόρτωση των ${verified ? "πιστοποιημένων" : "εκκρεμών"} ιατρών. Λεπτομέρειες: ${error.message}`,
          variant: "destructive",
        });
        setRows([]);
        setTotal(0);
      } else {
        setRows((data as DoctorProfile[]) ?? []);
        setTotal(count ?? 0);
      }
      setLoading(false);
    },
  ...
  const loadAuditLog = useCallback(async () => {
    ...
    if (error) {
      toast({
        ...
        description: `Δεν ήταν δυνατή η φόρτωση του ιστορικού. Λεπτομέρειες: ${error.message}`,
        variant: "destructive",
      });
      setAuditEntries([]);
    } else {
  ...
  const updateDoctor = async (id: string, patch: Partial<DoctorProfile>) => {
    ...
    setBusyId(id);
    const { error } = await supabase.from("doctor_profiles").update(patch).eq("id", id);
    setBusyId(null);
    if (error) {
      toast({
        title: `Δεν ήταν δυνατή η ενέργεια: ${actionLabel}`,
        description: `Το προφίλ του ${doctorName} δεν ενημερώθηκε. Δοκιμάστε ξανά σε λίγο. Λεπτομέρειες: ${error.message}`,
        variant: "destructive",
      });
      return;
    }
  ...
  const refetchClaim = async (id: string, attempt = 0): Promise<void> => {
    const { data, error } = await supabase
      ...
      .maybeSingle();
    if (error) {
      // Retry once with a small backoff before giving up.
      if (attempt < 1) {
        await new Promise((r) => setTimeout(r, 600));
        return refetchClaim(id, attempt + 1);
      }
      console.warn("Failed to refetch claim after rollback:", error.message);
      return;
    }
    if (!data) {
      // Claim no longer exists — drop it from local state.
      setClaims((prev) => prev.filter((c) => c.id !== id));
      setSelectedClaim((prev) => (prev && prev.id === id ? null : prev));
      return;
    }
    const fresh = data as ClinicClaim;
    setClaims((prev) => prev.map((c) => (c.id === id ? fresh : c)));
    setSelectedClaim((prev) => (prev && prev.id === id ? fresh : prev));
  };

  // Retry audit log insert with exponential backoff so we don't lose the
  // error_detail (or decision record) due to transient network/Supabase errors.
  const insertAuditLogWithRetry = async (
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      const { error } = await supabase.from("clinic_claim_audit_log").insert(payload);
      if (!error) return { ok: true };
      lastError = error.message;
      console.warn(
        `Audit log insert failed (attempt ${attempt}/${maxAttempts}) for claim ${payload.claim_id}: ${error.message}`,
      );
  ...
  const updateClaim = async (
    ...
    const { error } = await supabase
      .from("clinic_claims")
      .update({ status, decision_note: trimmedNote })
      .eq("id", id)
      .eq("status", "pending");
    setBusyId(null);
    if (error) {
      ...
      toast({
        ...
        description: `Το αίτημα (κατάσταση: ${statusLabel}) δεν ενημερώθηκε. Δοκιμάστε ξανά σε λίγο. Λεπτομέρειες: ${error.message}`,
        variant: "destructive",
      });
      // Audit log — record the failed attempt with the error reason so admins
      // can later inspect why the decision did not go through. Retried with
      // backoff to survive transient network errors.
      if (user?.id) {
        // Always normalize to one of the canonical failure decisions so the
        // audit log stays consistent and queryable.
        ...
        const auditResult = await insertAuditLogWithRetry({
          ...
          note: trimmedNote,
          error_detail: safeErrorDetail(error.message),
        });
        if (!auditResult.ok) {
          console.error(
            "Failed to write claim failure audit log after retries:",
            auditResult.error,
          );
          toast({
            title: "Προειδοποίηση: μη καταγραφή audit log",
            description: `Η αποτυχία της ενέργειας δεν καταγράφηκε στο ιστορικό. Λεπτομέρειες: ${auditResult.error ?? "άγνωστο σφάλμα"}`,
            variant: "destructive",
          });
        }
      }
      // Retryable refetch — ensure UI matches server truth even if the
      // optimistic rollback is stale (e.g. another admin decided concurrently).
  ...
  const exportAuditCsv = async () => {
    ...
    // Seed progress with what we know now: the filtered rows in the table give
    // a rough lower bound, but the server may return more (the table caps at
    // the most recent 500). Prefer the larger of the two so the bar grows
    // smoothly rather than overshooting 100% early.
    const initialEstimate = Math.max(filteredAuditEntries.length, 0) || null;
    setAuditExportProgress({
      rows: 0,
      bytes: 0,
      estimatedRows: initialEstimate,
      etaSeconds: null,
      elapsedMs: 0,
    });
    ...
    try {
      ...
      const url = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/export-audit-log-csv`;
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          from_iso: fromIso,
          to_iso: toIso,
          failures_only: auditFailuresOnly,
          claim_id: trimmedClaim || undefined,
          timezone_label: auditTimezone,
          bom: csvIncludeBom,
      ...
      const serverDelimiter = (res.headers.get("X-Audit-Delimiter") as CsvDelimiter | null) ?? csvDelimiter;
      const delimChar = serverDelimiter === "semicolon" ? ";" : serverDelimiter === "tab" ? "\t" : ",";
      const isHeaderLine = (line: string) =>
        line.startsWith(`created_at${delimChar}`) || line.startsWith("created_at (");

      // Stream the body so we can update a progress bar instead of blocking
      // until the whole CSV is buffered. We also count data rows incrementally
      // by tracking newlines in a line-buffer (handles chunks that split
      // mid-line). Falls back to text() if streaming isn't supported.
      let csvText: string;
      let dataRowCount = 0;
      ...
      if (res.body && typeof res.body.getReader === "function") {
        ...
        let bytes = 0;
        let lineBuffer = "";
        // Throttle React state updates to ~10/s to keep the UI smooth without
        // re-rendering on every chunk.
        let lastUiUpdate = 0;

        const countDataRow = (rawLine: string) => {
          // Strip BOM if present at the very start of the stream.
          const line = rawLine.replace(/^\uFEFF/, "");
          if (!line) return;
          if (line.startsWith("#")) return;
          if (line.startsWith("sep=")) return;
        ...
        while (true) {
          ...
          if (now - lastUiUpdate > 100) {
            ...
            const estimatedRows =
              initialEstimate && dataRowCount > initialEstimate ? dataRowCount : initialEstimate;
            // ETA: only after we have ≥1 data row and ≥500ms of samples,
            // and only if we have a meaningful estimate.
            let etaSeconds: number | null = null;
            if (
              estimatedRows &&
              dataRowCount > 0 &&
              elapsedMs > 500 &&
              dataRowCount < estimatedRows
            ) {
              const rate = dataRowCount / elapsedMs; // rows per ms
              etaSeconds = Math.max(0, Math.round((estimatedRows - dataRowCount) / rate / 1000));
            }
            setAuditExportProgress({
              rows: dataRowCount,
              bytes,
        ...
        // Flush decoder + any trailing line without a newline.
        ...
        if (lineBuffer.length > 0) {
          countDataRow(lineBuffer.replace(/\r$/, ""));
        }
        csvText = chunks.join("");
      } else {
        // Fallback: no streaming available; read everything at once.
        csvText = await res.text();
        dataRowCount = csvText
          .replace(/^\uFEFF/, "")
          .split(/\r?\n/)
          .filter(
            (line) =>
              line && !line.startsWith("#") && !line.startsWith("sep=") && !isHeaderLine(line),
          ).length;
      }
      ...
      const ext = serverDelimiter === "tab" ? "tsv" : "csv";
      const mime = serverDelimiter === "tab"
        ? "text/tab-separated-values;charset=utf-8;"
        : "text/csv;charset=utf-8;";
      const downloadBlob = new Blob([csvText], { type: mime });

      const dl = URL.createObjectURL(downloadBlob);
      ...
      toast({
        ...
        description: truncated
          ? "Το αποτέλεσμα περικόπηκε στο όριο ασφαλείας του server. Στενέψτε το χρονικό διάστημα."
          : "Το CSV δημιουργήθηκε από τον server με όλες τις εγγραφές που ταιριάζουν.",
      });
    } catch (err) {
...
const ClaimDetailsSheet = ({
  ...
  useEffect(() => {
    ...
    supabase
      .from("doctor_profiles")
      .select("full_name,specialty,email,phone,city,verified")
      .eq("user_id", claim.user_id)
      .maybeSingle()
      .then(({ data }) => {
        if (cancelled) return;
        setDoctor((data as DoctorLite | null) ?? null);
