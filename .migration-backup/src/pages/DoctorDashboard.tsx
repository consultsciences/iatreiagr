...
const DoctorDashboard = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const [{ data: prof }, { data: avail }, { data: bks }, { data: cls }] = await Promise.all([
        supabase.from("doctor_profiles").select("*").eq("user_id", user.id).maybeSingle(),
        supabase.from("doctor_availability").select("*").eq("user_id", user.id).order("day_of_week"),
        supabase.from("bookings").select("*").eq("doctor_id", user.id).order("appointment_date", { ascending: false }),
        supabase.from("clinic_claims").select("*").eq("user_id", user.id),
      ]);
      if (prof) setProfile({ ...empty, ...prof });
  ...
  const saveProfile = async () => {
    ...
    else toast({ title: "Αποθηκεύτηκε", description: "Το προφίλ ενημερώθηκε." });
  };
  ...
  const addSlot = async () => {
    ...
    const { data, error } = await supabase
      ...
      .insert({ user_id: user.id, ...newSlot })
      .select()
      .single();
    if (error) return toast({ title: "Σφάλμα", description: error.message, variant: "destructive" });
  ...
  const removeClaim = async (id: string) => {
    const { error } = await supabase.from("clinic_claims").delete().eq("id", id);
    if (error) return toast({ title: "Σφάλμα", description: error.message, variant: "destructive" });
