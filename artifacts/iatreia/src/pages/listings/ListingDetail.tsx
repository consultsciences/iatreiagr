...
import { z } from "zod";
import { ArrowLeft, MapPin, Mail, Phone, Loader2, Send, ShieldCheck } from "lucide-react";
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="border-b py-8" style={{ background: "var(--gradient-soft)" }}>
        <div className="container mx-auto px-4">
          ...
          <h1 className="mt-3 text-3xl font-bold tracking-tight md:text-4xl">{listing.title}</h1>
          <div className="mt-3 flex flex-wrap items-center gap-4 text-sm text-muted-foreground">
            {listing.city && (
              <span className="inline-flex items-center gap-1"><MapPin className="h-4 w-4" /> {listing.city}</span>
            )}
            {listing.price_label && (
          ...
          <div>
            ...
            <Card className="p-6">
              ...
              <p className="whitespace-pre-line text-sm leading-relaxed text-foreground/90">
                {listing.description || "Δεν υπάρχει αναλυτική περιγραφή. Επικοινωνήστε με τον καταχωρητή για περισσότερες πληροφορίες."}
              </p>
              {listing.meta && (
                <>
                  <h3 className="mt-6 mb-2 text-sm font-semibold uppercase tracking-wide text-muted-foreground">Χαρακτηριστικά</h3>
                  <p className="text-sm text-foreground/90">{listing.meta}</p>
                </>
              )}
            </Card>

            <Card className="mt-6 border-l-4 border-l-accent bg-accent/5 p-5 text-xs leading-relaxed text-muted-foreground">
              ...
              {(listing.contact_email || listing.contact_phone) && (
                <div className="mt-5 border-t pt-4 text-xs">
                  ...
                  {listing.contact_email && (
                    <div className="flex items-center gap-2 text-foreground/90"><Mail className="h-3.5 w-3.5" /> {listing.contact_email}</div>
                  )}
                  {listing.contact_phone && (
                    <div className="mt-1 flex items-center gap-2 text-foreground/90"><Phone className="h-3.5 w-3.5" /> {listing.contact_phone}</div>
                  )}
                </div>
              )}
            </Card>
